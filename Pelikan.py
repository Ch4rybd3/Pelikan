...
class IncidentInvestigationApp:
    def __init__(self, root):
        ...
        # Widgets de la partie gauche et droite
    ...
    def create_left_widgets(self):
        ...
        techAnalysis_frame = tk.LabelFrame(self.left_frame, text="Analyse Technique effectuées", bg="#d0d0d0")
        techAnalysis_frame.pack(fill=tk.BOTH, padx=10, pady=5, expand=True)
        self.techAnalysis_table = ttk.Treeview(techAnalysis_frame, columns=("Valeur", "Date/Heure"), show="headings")
        self.techAnalysis_table.heading("Valeur", text="Valeur")
        self.techAnalysis_table.heading("Date/Heure", text="Date/Heure")
        self.techAnalysis_table.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        # Ajouter une étape d'analyse technique
        add_techAnalysis_frame = tk.Frame(techAnalysis_frame, bg="#d0d0d0")
        add_techAnalysis_frame.pack(fill=tk.X, padx=10, pady=5)
        self.techAnalysis_entry = tk.Entry(add_techAnalysis_frame, width=50)
        self.techAnalysis_entry.pack(side=tk.LEFT, padx=5, pady=5)
        add_techAnalysis_button = tk.Button(add_techAnalysis_frame, text="Ajouter", command=self.add_techAnalysis)
        add_techAnalysis_button.pack(side=tk.LEFT, padx=5, pady=5)
        # Remédiation (tableau)
        ...
        add_remediate_button = tk.Button(add_remediate_frame, text="Ajouter", command=self.add_remediate)
    ...
    def save_metadata(self):
        ...
        for item in self.techAnalysis_table.get_children():
            values = self.techAnalysis_table.item(item, "values")
            techAnalysis = {
                "valeur": values[0],
                "date/heure": values[1]
            }
            metadata["analyse_technique"].append(techAnalysis)
        # Ajouter les étapes de remédiation à la liste
        for item in self.remediate_table.get_children():
            values = self.remediate_table.item(item, "values")
            remediate = {
                "valeur": values[0],
                "date/heure": values[1]
            }
    ...
    def load_incident(self, incident_folder):
        ...
        for techAnalysis in metadata["analyse_technique"]:
           self.techAnalysis_table.insert("", "end", values=(techAnalysis["valeur"], techAnalysis["date/heure"]))
        # Charger les étapes de remédiation dans la table
        for remediate in metadata["remédiation"]:
            self.remediate_table.insert("", "end", values=(remediate["valeur"], remediate["date/heure"]))
    ...
    def add_techAnalysis(self):
        techAnalysis_value = self.techAnalysis_entry.get()
        if not techAnalysis_value:
            messagebox.showwarning("Attention", "Veuillez saisir une valeur pour l'étape d'analyse technique.")
            return
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.techAnalysis_table.insert("", "end", values=(techAnalysis_value, now))
        self.techAnalysis_entry.delete(0, tk.END)

    def add_remediate(self):
        remediate_value = self.remediate_entry.get()
        if not remediate_value:
            messagebox.showwarning("Attention", "Veuillez saisir une valeur pour l'étape de remédiation.")
            return
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.remediate_table.insert("", "end", values=(remediate_value, now))
        self.remediate_entry.delete(0, tk.END)
